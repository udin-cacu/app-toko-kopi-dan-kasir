  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-2xl shadow">
      ...
      <div class="modal-body text-center">
        <div id="qrisBox">Memuat QRIS...</div>
        <div class="mt-3">
          <small>Silakan scan QR untuk menyelesaikan pembayaran</small>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  // cek metode pembayaran
    if(payload.method === 'qris'){
      $.post("{{ route('pos.paymen') }}", payload).done(res=>{
    // Panggil Midtrans Snap popup
        snap.pay(res.snap_token, {
          onSuccess: function(result){
            $('#info').html(`<div class="alert alert-success">Pembayaran berhasil! Invoice: <b>${res.invoice}</b></div>`);
            cart.length = 0; render();
          },
          onPending: function(result){
            $('#info').html(`<div class="alert alert-warning">Menunggu pembayaran QRIS...</div>`);
          },
      ...
      }).fail(()=> alert('Gagal generate QRIS'));
